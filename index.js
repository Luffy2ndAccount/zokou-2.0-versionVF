...
setTimeout(() => {
    async function main() {
        ...
        setInterval(() => { store.writeToFile(__dirname + "/store.json"); }, 1000);
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            if (texte && (mtype == "conversation" || mtype == "extendedTextMessage") && !verifCom) {
                ...
                if (origineMessage === "status@broadcast") return;


                const afkfunc = require("./bdd/afk");

                if (origineMessage.endsWith("@s.whatsapp.net")) {

                    let afkSettings = await afkfunc.getAfkById(1) ? await afkfunc.getAfkById(1)?.etat : "non";
                    ...
                    if (afkSettings == "on") {
                        ...
                        if (ms.key.fromMe) {
                            ...
                            await afkfunc.changeAfkState(1, "off");
                            repondre("Etant donnee que vous etes de retour nous desactivons le Afk");

                        } else {

                            if (afkSettings.lien == "no url") {

                                repondre(afkSettings.message);

                            } else {

                                zk.sendMessage(origineMessage, { image: { url: afkSettings.lien }, caption: afkSettings.message }, { caption: ms });

                            }

                        }

                    } else if (conf.CHATBOT === "oui") {

                        if (superUser || ms.key.fromMe) return;

                        const traduire = require("./framework/traduction");

                        let message = await traduire(texte, { to: 'en' });
                    ...
                    let afkSettings = await afkfunc.getAfkById(1);
                    ...
                    if (afkSettings?.etat == "on") {
                        ...
                        if (ms.key.fromMe) {
                            ...
                            await afkfunc.changeAfkState(1, "off");
                            return repondre("Etant donnee que vous etes de retour nous desactivons le Afk");

                        }
                    };

                    if (ms.message[mtype]?.contextInfo?.mentionedJid && ms.message[mtype]?.contextInfo?.mentionedJid.includes(idBot)) {
...
*/
        ...
        zk.getLastMessageInChat = async (jid) => {

            let filepath = `store.json`;

            let text = fs.readFileSync(filepath, 'utf-8');

            let json = JSON.parse(text);

            if (json?.messages && json?.messages[jid]) {

                return json.messages[jid][json.messages[jid].length - 1];
            } else {
